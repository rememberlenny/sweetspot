...
function init(){
    ...
    // $.ajax('http://getsweetspot.herokuapp.com/stories/1.json?callback=?', function(data) {
    //     storyData = data;
    //
    //     renderStoryImage($container, storyData[0]);
    // })
    //
    //
    console.log(mock_data.photos[0]);
    renderStoryImage($container, mock_data.photos[0]);
}
...
function renderStoryImage($el, storyObj) {
    var isFirst = false;
    if (mock_data.photos.indexOf(storyObj) == 0) {
        isFirst = true;
    }
...
function storeOldStoryImage($el) {
    ...
    var storyImageObj = _.findWhere(mock_data, {id: imgName});
...
function hotspotClick(e) {
    ...
    var oldStoryImage = _.findWhere(mock_data, {id: oldImgName});
    ...
    var newStoryImage = _.findWhere(mock_data.photos, {id: newImgName});
